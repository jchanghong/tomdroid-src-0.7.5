...
import org.changhong.sync.baidu.FileTime;
import org.changhong.ui.Tomdroid;
import org.changhong.util.ErrorList;
import org.changhong.util.Preferences;
import org.changhong.util.TLog;
import org.changhong.util.Time;
...
public class SdCardSyncService extends SyncService {
	...
	// logging related
	private final static String TAG = "SdCardSyncServiceAAAAAAA";
	...
	private class Worker implements Runnable {
		...
		public void run() {
            String filename;
            filename = FileTime.nameToUUL(file.getName());
//			note.setFileName(file.getAbsolutePath());
            note.setFileName(FileTime.nameToUUL(file.getAbsolutePath()));
            // the note guid is not stored in the xml but in the filename
            note.setGuid(filename.replace(".note", ""));
	...
	private static int doPushNote(Note note) {
		...
		try {
            ...
            String paths = FileTime.uulToName(Tomdroid.NOTES_PATH + "/" + note.getGuid() + ".note");
